...
export const getAdminPlaylists = (partyId: string, signal?: AbortSignal) =>
  apiRequest<PlaylistListResponse>(`/api/organizer/parties/${partyId}/playlists`, {
    ...(signal === undefined ? {} : { signal }),
  });

export const createAdminPlaylist = (partyId: string, input: CreatePlaylistRequest) =>
  apiRequest<PlaylistResponse>(`/api/organizer/parties/${partyId}/playlists`, {
    method: "POST",
    body: input,
...
export const updateAdminPlaylist = (playlistId: string, input: UpdatePlaylistRequest) =>
  apiRequest<PlaylistResponse>(`/api/organizer/playlists/${playlistId}`, {
    method: "PATCH",
    body: input,
    csrfCookie: "songfest_admin_csrf",
  });
...
export const activateAdminPlaylist = (playlistId: string) =>
  apiRequest<PlaylistResponse>(`/api/organizer/playlists/${playlistId}/activate`, {
    method: "POST",
    csrfCookie: "songfest_admin_csrf",
  });

export const deleteAdminPlaylist = (playlistId: string) =>
  apiRequest<void>(`/api/organizer/playlists/${playlistId}`, {
    method: "DELETE",
    csrfCookie: "songfest_admin_csrf",
  });
